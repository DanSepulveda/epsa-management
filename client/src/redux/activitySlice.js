...
export const createActivity = createAsyncThunk('createActivity', async ({ values, token }) => {
...
export const getActivities = createAsyncThunk('getActivities', async (token) => {
...
export const editActivity = createAsyncThunk('editActivity', async (data) => {
...
export const deleteActivity = createAsyncThunk('deleteActivity', async (data) => {
  const { id, token } = data
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(editActivity.fulfilled, (state, action) => {
        ...
        if (success) {
          state.activities = [
            ...state.activities.filter((activity) => activity._id !== response._id),
            response,
      ...
      .addCase(deleteActivity.fulfilled, (state, action) => {
        ...
        if (success) {
          state.activities = [
            ...state.activities.filter((activity) => activity._id !== response),
          ].sort((a, b) => a.name.localeCompare(b.name))
        }
        state.loading = false
      })
